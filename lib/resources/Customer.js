...
function Customer(json, apiMethod) {
  if(!(this instanceof Customer)) {
    return new Customer(json, apiMethod);
  }
...
Customer = _.extend(Customer, {
  ...
  generateInvoices: function(id, params, headers, callback) {
    var Invoice = require('./Invoice');
    var methodArgs = APIResource.prepareArguments(arguments, Customer.generateInvoice);
    return new APIMethod("post", "/customers/:id/generate_invoices", methodArgs, undefined, APIList.constructBldr(Invoice)).execute();
  },
...
Customer.prototype = {
  ...
  generateInvoices: function(params, headers, callback) {
    var Invoice = require('./Invoice');
    var methodArgs = APIResource.prepareArguments(arguments, Customer.prototype.generateInvoice);
    return new APIMethod("post", "/customers/:id/generate_invoices", methodArgs, this, APIList.constructBldr(Invoice)).execute();
  },
